...
#
# Ограничения AWK парсера:
#   - Работает только с простым форматом: "- name: xxx" + "src: yyy"
#   - Не поддерживает кавычки в значениях (будут включены в результат)
#   - Не поддерживает многострочные значения
#   - Не поддерживает YAML алиасы и якоря
#   - При дублировании key в одном блоке берётся последнее значение
parse_emoji_yaml() {
    local yaml_content="$1"

    # AWK скрипт для парсинга простой YAML структуры
    # Алгоритм:
    #   1. Пропускаем пустые строки и комментарии
    #   2. При встрече "- name:" или "name:" сохраняем имя в current_name
    #   3. При встрече "src:" сохраняем URL и выводим пару name|src
    #   4. Сбрасываем current_name после успешного вывода
    echo "$yaml_content" | awk '
        # Пропускаем пустые строки и комментарии
        /^[[:space:]]*$/ { next }
        /^[[:space:]]*#/ { next }

        # Обрабатываем строку с именем эмодзи (начало нового элемента списка)
        /^[[:space:]]*- name:/ {
        ...
        # Обрабатываем строку с именем эмодзи (продолжение списка, без дефиса)
        /^[[:space:]]*name:/ {
            gsub(/^[[:space:]]*name:[[:space:]]*/, "")
            gsub(/[[:space:]]*$/, "")
            current_name = $0
